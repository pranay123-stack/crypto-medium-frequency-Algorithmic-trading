...
def get_candle_df(coinbase,symbol,timeframe,limit=20):
    ...
    Uses ccxt coinbase instead of cbpro
    '''
    # Convert timeframe from seconds to ccxt format (e.g., 300 -> '5m', 900 -> '15m')
    tf_map = {60: '1m', 300: '5m', 900: '15m', 3600: '1h', 86400: '1d'}
    tf = tf_map.get(timeframe, '5m')
    # Convert symbol format (ETH-USD -> ETH/USD)
    ccxt_symbol = symbol.replace('-', '/')
    ohlcv = pd.DataFrame(coinbase.fetch_ohlcv(ccxt_symbol, tf, limit=limit),
